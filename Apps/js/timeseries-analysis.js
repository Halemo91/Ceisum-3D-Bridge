...
var start = Math.floor(options.period/2);
var end = options.period - start;
// Leave the datapoints [0;start + 1[ intact
this.buffer = this.data.slice(0, start + 1);
sum = 0;
for(i = 0; i < options.period; i++){
sum += this.data[i][1];
}
this.buffer.push([this.data[i - start][0], sum/options.period]);

for (i=start + 1;i< l - end + 1;i++) {
sum += - this.data[i - start - 1][1] + this.data[i + end - 1][1];
console.log(i+end);
this.buffer.push([this.data[i][0], sum/options.period]);
}
for(i = l - end; i < l; i++){
this.buffer.push(this.data[i]);
}
this.data = this.buffer;
return this;
}
...
var l = this.data.length;
var sum	= 0;
var n	= (options.period*(options.period))/2;
...
this.buffer.push([this.data[i][0], sum/n]);
}
for(i = l - end; i < l; i++){
this.buffer.push(this.data[i]);
}
